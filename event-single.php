...
if (isset($_POST['submit2'])) {
  $eventid = mysqli_real_escape_string($usman_ngo, $_POST['eventid']);
  ...
  if (empty($_POST['name']) || empty($_POST['email']) || empty($_POST['message'])) {
    ...
    echo "<script>window.location.href='./event-single.php?eventid=" . $eventid . "';</script>";
  }
//  print_r($eventid);die;
  $query = sprintf("INSERT INTO `event_comment`(`event_id`, `name`, `email`, `website`, `message`) VALUES ('$eventid','$name','$email','$website','$message')");
  //  print_r($query);die;
...
<body>
    ...
    <div class="container">
      <div class="row no-gutters slider-text align-items-center justify-content-center" data-scrollax-parent="true">
        <div class="col-md-7 ftco-animate text-center" data-scrollax=" properties: { translateY: '70%' }">
          <p class="breadcrumbs" data-scrollax="properties: { translateY: '30%', opacity: 1.6 }"><span class="mr-2"><a href="index.php">Home</a></span> <span class="mr-2"><a href="blog.php">Event</a></span> <span>Event Details</span></p>
  ...
  <section class="ftco-section ftco-degree-bg">
    <div class="container">
      <div class="row">
        <?php
        $faculty = $usman_ngo->query("SELECT * FROM `event2` WHERE `id`= '{$event_id}'");
        $row = $faculty->fetch_array();
        ?>
        <div class="col-md-8 ftco-animate">
          <h2 class="mb-3"><a class="text-dark" href="#"><?php echo ucwords($row['header']) ?></h2>
          ...
          <div class="pt-5 mt-5">
            <?php
            $result = $usman_ngo->query("SELECT COUNT(*) FROM `event_comment` WHERE `event_id`= '{$event_id}'");
            $row = $result->fetch_row();
            $count = $row[0]; ?>
            <h3 class="mb-5"><?php echo $count ?> Comments</h3>
            <ul class="comment-list">
              <?php
              $comments = $usman_ngo->query("SELECT * FROM `event_comment` WHERE `event_id`= '{$event_id}'");
